"""
S3: Mixed Push/Pop Workload (Producer-Consumer Pattern) - Direct Actor Version
...
Uses Dapr Actor SDK to invoke actors directly (bypassing REST API layer).

Variants:
- S3a: Balanced (50% push, 50% pop)
- S3b: Producer-heavy (80% push, 20% pop)
- S3c: Consumer-heavy (20% push, 80% pop)
- S3d: Multi-priority mixed workload
- S3e: Balanced with large items
...
from scenarios.base_dapr import BaseDaprActorUser


class MixedWorkloadDaprUser(BaseDaprActorUser):
    """
...
class ProducerHeavyDaprUser(BaseDaprActorUser):
    """
    ...
    abstract = False
    wait_time = between(0.00, 0.01)  # Average 0.1s = ~10 RPS per user
...
class ConsumerHeavyDaprUser(BaseDaprActorUser):
    """
...
class MultiPriorityMixedDaprUser(BaseDaprActorUser):
    """
...
class BalancedLargeItemsDaprUser(BaseDaprActorUser):
    """
...
# Export all variants
__all__ = [
    "MixedWorkloadDaprUser",           # S3a - Most important, default
    "ProducerHeavyDaprUser",           # S3b - Growing queue
    "ConsumerHeavyDaprUser",           # S3c - Draining queue
    "MultiPriorityMixedDaprUser",      # S3d - Priority testing
    "BalancedLargeItemsDaprUser",      # S3e - Payload size testing
]
